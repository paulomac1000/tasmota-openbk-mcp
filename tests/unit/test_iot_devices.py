...
class TestGetDeviceInfo:
    ...
    def test_get_device_info_by_ip(self):
        ...
        with (
            patch("tools.iot_discovery._resolve_ip", return_value="192.168.1.100"),
            patch(
                "tools.iot_discovery._detect_device_type",
                return_value="tasmota",
            ),
            patch("tools.iot_devices._get_tasmota_status") as mock_status,
        ):
    ...
    def test_get_device_info_name_not_found(self):
        ...
        with (
            patch("tools.iot_discovery._resolve_ip", return_value=None),
            patch("tools.iot_discovery._get_cached_devices", return_value=[]),
        ):
    ...
    def test_get_device_info_openbk(self):
        ...
        with (
            patch("tools.iot_discovery._resolve_ip", return_value="192.168.1.101"),
            patch(
                "tools.iot_discovery._detect_device_type",
                return_value="openbk",
            ),
            patch("tools.iot_devices._get_openbk_status") as mock_status,
        ):
            mock_status.return_value = {
                "name": "OpenBK_Test",
                "channels": [],
    ...
    def test_get_device_info_device_not_found_at_ip(self):
        ...
        with (
            patch("tools.iot_discovery._resolve_ip", return_value="192.168.1.200"),
...
class TestGetDevicePower:
    ...
    def test_get_device_power_tasmota(self):
        ...
        with (
            ...
            patch("tools.iot_devices.requests.get") as mock_get,
        ):
            resp = MagicMock()
            resp.status_code = 200
            resp.json.return_value = {"POWER": "ON"}
...
class TestGetDeviceInfoTuyaErrors:
    ...
    def test_get_device_info_tuya_success(self):
        ...
        with (
            ...
            patch("tools.iot_devices._get_openbk_status"),
            patch("tools.iot_devices._get_tasmota_status"),
            patch("tools.iot_tuya._tuya_status") as mock_tuya,
        ):
    ...
    def test_get_device_info_tuya_status_error(self):
        ...
        with (
            ...
            mock_tuya.return_value = json.dumps({"success": False, "error": "Device offline"})
            result = _get_device_info("192.168.1.100")
            data = json.loads(result)
            assert data["success"] is False
...
class TestGetDeviceInfoOpenHASPErrors:
    ...
    def test_get_device_info_openhasp_success(self):
        ...
        with (
            ...
            patch("tools.iot_devices._get_openbk_status"),
            patch("tools.iot_devices._get_tasmota_status"),
            patch("tools.iot_openhasp._openhasp_status") as mock_hasp,
        ):
    ...
    def test_get_device_info_openhasp_status_error(self):
        ...
        with (
            ...
            mock_hasp.return_value = json.dumps({"success": False, "error": "Connection refused"})
            result = _get_device_info("192.168.1.100")
            data = json.loads(result)
            assert data["success"] is False
